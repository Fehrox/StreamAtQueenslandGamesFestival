...
```text
Timestamp | Creator Id | Creator Name | Creator Email | Stream Station | Twitch Connect | Panel Interest | Twitch OAuth Login | IRL Streaming | Deployment Id | Source | Notes
```

The current site submits rows in this exact order:

| Sheet column | Submitted field |
| --- | --- |
| Timestamp | Generated by Apps Script |
| Creator Id | `creatorId` |
| Creator Name | `creatorName` |
| Creator Email | `creatorEmail` |
| Stream Station | `streamingStation` |
| Twitch Connect | `twitchConnect` |
| Panel Interest | `panelInterest` |
| Twitch OAuth Login | `twitchOAuthLogin` |
| IRL Streaming | `irlStreaming` |
| Deployment Id | `deploymentId` |
| Source | `source` |
| Notes | Blank, for manual notes |

Do not include `Travel Funding` or `User Agent` columns. The page no longer
collects or submits those values.
...
function doPost(e) {
  try {
    ...
    sheet.appendRow([
      ...
      text_(params.twitchOAuthLogin).toLowerCase(),
...
Submissions confirmed through Twitch populate `Twitch OAuth Login` with the login returned by Twitch. Submissions confirmed without account verification leave that column blank.
